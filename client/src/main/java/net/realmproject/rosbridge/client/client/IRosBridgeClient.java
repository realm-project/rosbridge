...
/**
 * This class provides the functionality defined in the {@link RosBridgeClient}
 * interface by using a system of {@link Consumer} handlers which can register
 * to listen for specific opcodes and take action as required.
 * {@link RosBridgeCall}s and {@link RosBridgeSubscriber}s are forms of
 * {@link Consumer} handlers
 * 
 * @author nathaniel
 *
 */
...
public class IRosBridgeClient implements RosBridgeClient {
    ...
    private void handleMessage(RosBridgeMessage message) {
        ...
        // get a copy of the handlers, so that if one removes itself, we don't
        // get concurrent mod ex
    ...
    @Override
    public RosBridgeSubscriber<?> subscribe(String topic, String type, Consumer<Object> handler)
            throws InterruptedException, IOException {
    ...
    @Override
    public <T> RosBridgeSubscriber<T> subscribe(String topic, Class<T> clazz, Consumer<T> handler)
            throws InterruptedException, IOException {
    ...
    @Override
    public <T> RosBridgeSubscriber<T> subscribe(String topic, String type, Class<T> clazz, Consumer<T> handler)
            throws InterruptedException, IOException {

        // create new subscription, and add it as a handler for the "publish"
        // opcode
    ...
    @Override
    public void call(final String service, final List<?> args, final Consumer<Map<String, Object>> handler)
            throws InterruptedException, IOException {
        Map<String, Object> sample = new HashMap<>();
    ...
    @Override
    public <T> void call(String service, List<?> args, Class<T> clazz, Consumer<T> handler)
            throws InterruptedException, IOException {
        ...
        RosBridgeCall<T> call = new IRosBridgeCallbackCall<T>(service, id, clazz, handler,
                new Consumer<RosBridgeCall<T>>() {
    ...
    public void unadvertise(RosBridgePublisher publisher) throws InterruptedException, IOException {
        ...
        // make sure this is a valid publisher, then remove it from the set of
        // valid publishers
        ...
        connection.sendMessage(RosBridgeSerialize.serialize(message));
